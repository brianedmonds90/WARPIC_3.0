...
public class WarpicActivity extends PApplet{ //PApplet in fact extends android.app.Activity
	...
	Menu menu;
	static boolean showController,fingersOnScreen,animate,firstPass,saveWarp;
	...
	public void setup() {
		...
		Parse.initialize(this, "BgpuXDtTXuLyXOyv5A6sEs6ehKqcaXakOC7Mg13N", "A8pFBNRcLjGrhclpFFvJFNXsMaZyOfZypjiKlPjD");
		testObject = new ParseObject("TestObject");
		motionStrBuilder= new StringBuilder();
		motionString="";
		motionFile = new File(Environment.getExternalStorageDirectory() ,"WARPIC/motion.txt");	
		motionFile.delete();
		motionFile = new File(Environment.getExternalStorageDirectory() ,"WARPIC/motion.txt");	
		...
		imageMode(PORTRAIT);	
		fingersOnScreen=false; firstPass=true; animate=false; saveWarp=false; grabPoints=false;
    	...
    	showController=true; fingersOnScreen=false; writePaths=true;
	...
	public void draw() {
    	...
    	synchronized(l){//handle the previously queued motion events
    		if(!showMenu){
				...
				for(MyMotionEvent me: l)//Register the button input
...
//    	    	motionString+=displayInfo;
    	...
    	try{
    		if(mController!=null&&mController.size()==4&&showController){
	...
	private PImage goGetImage(String string){
		...
		try{
			...
			bMap.getPixels(img.pixels, 0,img.width, 0, 0, img.width, img.height);//Store the bMap's pixels into a pixel array
			img.updatePixels();
    ...
    private void animateWarping() {
    	...
    	tracking=computeAnimationFrame(currentT, mController.getElapsedTime(), numFrames);
    ...
    private void setPairsDev() {
		if(mController.getHistoryOf(0).size()>0&&mController.getHistoryOf(1).size()>0
				&&mController.getHistoryOf(2).size()>0&&mController.getHistoryOf(3).size()>0){
			...
			L= new Pair(listA.get(0), listB.get(0), listA.get(listA.size()-1), listB.get(listB.size()-1));
			R=new Pair(listC.get(0), listD.get(0), listC.get(listC.size()-1), listD.get(listD.size()-1));		
		}
	}
 ...
 /*******************************************************************************************/
  public boolean surfaceTouchEvent(MotionEvent me) {//Overwrite this android touch method to process touch data
    ...
    if(action==1&&finger<pointerCount){ 
      ...
      loc = new Pt(px, py);
      synchronized(l){
      l.add(new MyMotionEvent(action, finger, loc, pointerCount,System.nanoTime())); //queue all the data recieved from the motion events
      }
    } 
    else if(action==0){
      if(pointerCount==1){
      ...
      synchronized(l){
        for(int i=0;i<me.getPointerCount();i++){
          ...
          float py = me.getY(i);
          loc = new Pt(px, py);
          l.add( new MyMotionEvent(action, me.getPointerId(i), loc, pointerCount,System.nanoTime())); //queue all the data recieved from the motion events
        } 
      }
    } 
  ...
  public void keyPressed() {//Display menu
	  if (key == CODED) {
	    if (keyCode == KeyEvent.KEYCODE_MENU) {//When the menu button is pressed the animation begins
  ...
  void warpVertices(Pair L, float f, float roi) { 
	  L.prepareToWarp(roi,this); // precompute some values that are the same for each call to warpDirectly
   for (int i=0; i<n; i++) 
	   for (int j=0; j<n; j++) 
...
//transform 
  ...
  Pt P(Pt A, Pt B, Pt C, Pt D) {return average(average(A,B),average(C,D)); };                                            // (A+B+C+D)/4 (average)
  ...
  Pt P(double x, double y) {return new Pt((float)x,(float)y); };
...
/*********************************************Smoothing***************************/
  ...
  void tuck(float s,ArrayList<Pt> list) {
		    ...
		    for (int v=1; v<list.size()-1; v++) S[v]=P(list.get(v),s,average(list.get(v-1),list.get(v+1)));  // S = G + s((P+N)/2-G)
		    for (int v=1; v<list.size()-1; v++) list.set(v,S[v]); // copy back
		 }
/*********************************************End of Smoothing***************************/
  ...
  public void timeToAnimate() {
	   getReadyToAnimate();
	   smooth(A);smooth(B);smooth(C);smooth(D);
	   animate=true;
	   showController=false;
	   showSpirals=false;
	   showMenu=false;
                                 ...
                                 testObject.saveInBackground(new SaveCallback() {
                                       public void done(ParseException e) {
                                         if (e == null) {
                                                 ...
                                                 String url="http://ec2-54-237-34-141.compute-1.amazonaws.com/?"+urlID;
                                                 launchShareUrl(url);
                                    ...
                                    private void launchShareUrl(String url) {
                                              ...
                                              startActivity(Intent.createChooser(shareIntent, "Share your Animation!!"));
                                    }
                            });
  ...
  private byte[] parseImageFile() {
	  Bitmap bMap;
	  bMap = BitmapFactory.decodeFile(imageFilePath);	//Get the jpeg image as a Bitmap from the filesystem
